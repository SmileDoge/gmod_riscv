...
void Update(GmodEmulator* emu, IDevice* dev)
{
    if (is_ready)
		return;

    GmodMachine* machine = emu->CreateMachine(1, 1, 128 * 1024 * 1024);

    if (machine)
        RV_INFO("Machine created");
    else
    {
        RV_ERROR("CreateMachine error");
        return;
    }

	machine->WaitForReady(200);

    if (machine->IsReady())
    {
        RV_INFO("Machine is ready!");
        is_ready = true;

		machine->SetBIOSPath("D:\\gmod_server_turbostroi_64\\fw_jump.bin");
		machine->SetKernelPath("D:\\gmod_server_turbostroi_64\\Image");

        machine->AppendCommandLine("root=/dev/nvme0n1 rootflags=discard rw");
	
        machine->AttachDevice(dev, 0x1000'0000);

		machine->DumpDTB("test_fdt.dtb");
    
		machine->StartResume();
    }
    else
    {
		RV_ERROR("Machine is not ready! Timeout"); // timeout ?

		emu->DestroyMachine(1);
    }
}

int main(int argc, char** argv)
{
	GmodEmulator* emu = new GmodEmulator();

	emu->Start(nullptr);

    emu->RegisterDevice("simple_device", CreateSimpleDevice, nullptr, nullptr);

    nlohmann::json json_empty;

    //IDevice* dev = emu->CreateDevice("simple_device", json_empty);

    //RV_INFO("Device - type: '%s' id: %d realm: %d size: %d", dev->GetName(), dev->GetRealm(), dev->GetSize());

    while (true)
    {
        emu->Update();

        if (emu->GetState() == EmulatorState::CHILD_PROCESS_EXITED)
            break;
...
GmodEmulator* g_Emulator = nullptr;

LUA_FUNCTION(GmodEmulatorUpdate)
{
    if (g_Emulator)
        g_Emulator->Update();
...
GMOD_MODULE_OPEN()
{
	g_Emulator = new GmodEmulator();

	GmodEmulatorLua::Initialize(LUA, g_Emulator);
	GmodMachineLua::Initialize(LUA, g_Emulator);
    GmodDeviceLua::Initialize(LUA, g_Emulator);
    ...
    g_Emulator->NotifyAllRegister(LUA);
	...
	LUA->PushSpecial(GarrysMod::Lua::SPECIAL_GLOB);
        LUA->GetField(-1, "hook");
            LUA->GetField(-1, "Add");
                ...
                LUA->PushCFunction(GmodEmulatorUpdate);
			LUA->Call(3, 0);
        LUA->Pop();
    LUA->Pop();
...
GMOD_MODULE_CLOSE()
{
    g_Emulator->NotifyAllUnregister(LUA);
